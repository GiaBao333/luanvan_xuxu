		...
		<div class="row">
			<div class="col-lg-6">
				<div class="form-group">
					<a href="{{ url('/ap/create') }}" title=""><button type="button" class="btn btn-default">Thêm</button></a>
				</div>
                        <table class="table table-bordered table-hover">
                        	...
                        	<tbody>
                        	<?php $stt = 1; ?>
                        	@foreach(Ap::all() as $value)
                        		<tr>
                        			<td>{{ $stt }}</td>
                        			<td>{{ $value->ten }}</td>
                        			<td>{{ Ap::FindAddress($value->xa_phuong_id) }}</td>
                        			<td>
                        				<button type="button" data-delete="{{ $value->id }}" class="btn btn-default btn-delete">
                                            <i class="glyphicon glyphicon-trash"></i>
                                        </button>
                                        	<a href="{{ url('/ap/'.$value->id.'/edit') }}" title="">
